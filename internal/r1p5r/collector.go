...
type ProviderClient interface {
	ServerTime(context.Context) (time.Time, string, prospective.ResponseEvidence, error)
	KlinesRange(context.Context, string, int64, int64) (prospective.ResponseEvidence, string, error)
}
...
func NewCollector(config Config) *Collector {
	return &Collector{Config: config, Store: NewStore(config.DataRoot, config.Protocol, config.SourceIdentity, config.PreacquisitionSeal), Client: prospective.NewClient(), Clock: prospective.SystemClockChecker{}, Now: func() time.Time { return time.Now().UTC() }, Delay: 120 * time.Millisecond}
}
...
func (c *Collector) Status() (BackfillStatus, error) {
	...
	status := BackfillStatus{SchemaVersion: "ak-historian.pr4b0-r1p5r.backfill-status.v1", GeneratedAtUTC: c.Now().UTC(), Complete: true, Requests: len(entries), ChainTerminal: state.ChainTerminal, Cursors: state.Cursors}
...
func (c *Collector) collectPage(ctx context.Context, state State, symbol string, start, end time.Time) (LedgerEntry, State, error) {
	...
	if evidence.RequestStartUTC.Before(c.Config.PreacquisitionSeal.VerificationCompletedAtUTC) {
		return LedgerEntry{}, State{}, errors.New("acquisition request predates preacquisition source-seal verification")
	}
	...
	requestID := prospective.HashBytes([]byte(c.Config.PreacquisitionSeal.SourceSealCommit + "\n" + c.Config.PreacquisitionSeal.BinarySHA256 + "\n" + c.Config.Protocol.ProtocolHash + "\n" + symbol + "\n" + start.Format(time.RFC3339) + "\n" + end.Format(time.RFC3339)))
